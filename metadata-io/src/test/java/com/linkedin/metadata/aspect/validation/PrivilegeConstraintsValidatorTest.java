...
public class PrivilegeConstraintsValidatorTest {
  ...
  public void testValidateGlobalTagsPatchAuthorized() throws Exception {
    ...
    GenericJsonPatch genericJsonPatch =
        GenericJsonPatch.builder()
            ...
            .build();
    mcp.setAspect(serializePatch(genericJsonPatch));
    ...
    ProposedItem proposedItem =
        ProposedItem.builder()
            ...
            .entitySpec(entityRegistry.getEntitySpec(mcp.getEntityType()))
            .build();
  ...
  // Upstream calls GenericRecordUtils.serializePatch(GenericJsonPatch, ObjectMapper); that overload
  // does not exist on this branch's base, so the patch aspect is assembled here instead.
  private GenericAspect serializePatch(GenericJsonPatch genericJsonPatch) throws Exception {
    GenericAspect genericAspect = new GenericAspect();
    genericAspect.setValue(
        ByteString.unsafeWrap(
            objectMapper.writeValueAsString(genericJsonPatch).getBytes(StandardCharsets.UTF_8)));
