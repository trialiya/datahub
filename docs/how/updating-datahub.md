...
- A dataset's `upstreamLineage` aspect is now governed by its own privileges when accessed through the entity APIs (OpenAPI v1/v2/v3 and Rest.li), instead of by the entity-level privileges. The privileges match, operation for operation, those the graph-based lineage endpoints already use (the `LINEAGE` API group), plus the new `VIEW_LINEAGE` privilege as a dedicated, narrowest read grant: reading the aspect accepts `VIEW_LINEAGE`, `VIEW_ENTITY_PAGE`, `GET_ENTITY`, `EDIT_ENTITY`, `EDIT_LINEAGE` or `DELETE_ENTITY`; creating/updating it accepts `EDIT_LINEAGE` or `EDIT_ENTITY`; deleting it additionally accepts `DELETE_ENTITY`. Each is sufficient on its own — `EDIT_LINEAGE` alone can now write the aspect without `EDIT_ENTITY`, and `VIEW_LINEAGE` alone can read it without any entity-level read privilege. Because these privileges replace the entity-level ones for this aspect rather than being combined with them, an actor holding **none** of them cannot read or write `upstreamLineage` even where an entity-level privilege would otherwise have allowed it.

  Endpoints returning **multiple aspects** — "get entity", batch gets, and search/scroll responses that embed aspects — omit the aspect silently when the caller lacks the privilege, whether it was named explicitly in the aspect list or came from a wildcard. The response looks exactly as it would if the aspect were unset, so listing or searching never fails part-way through on account of one unreadable aspect. Only endpoints addressing a **single named aspect of a single entity** (`GET /openapi/v3/entity/dataset/{urn}/upstreamLineage` and its create/patch/delete counterparts, the Rest.li `/aspects` resource) answer with 403, since there the aspect is the entire request.
